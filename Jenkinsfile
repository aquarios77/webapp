pipeline{ // start of the declarative pipeline
    // Directives  - agent, tools, environment, etc.
    agent any // will try to execute this pipeline on any available agent (defines where the job will be executed)
    tools { // 
        maven 'maven'
    }
    environment{
    ...
    stages {
        ...
        stage ('Build'){
            steps {
                sh 'mvn clean install package' // executing the shell command
            }
        }
        ...
        stage ('Test'){
            steps {
                echo ' testing......' // emulating some testing routine
        ...
        stage ('Publish to Nexus'){
            steps {
                ...
                def NexusRepo = Version.endsWith("SNAPSHOT") ? "AntonsDevOpsLab-SNAPSHOT" : "AntonsDevOpsLab-RELEASE"
